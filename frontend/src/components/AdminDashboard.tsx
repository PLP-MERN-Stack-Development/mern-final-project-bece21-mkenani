...
const AdminDashboard: React.FC = () => {
  ...
  // --- NEW --- State for the announcement form
...
// Function to handle updating education level
  ...
  const handleSendAnnouncement = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setSendSuccess(true);
      setAnnouncementMessage(''); // Clear message on success
  ...
  return (
    <div className="max-w-7xl mx-auto py-6">
      ...
      {stats && (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <StatCard 
            ...
            icon={<UserCheck className="w-6 h-6 text-blue-600 dark:text-blue-300" />} 
          />
          <StatCard 
            title="Total Groups" 
            value={stats.totalGroups} 
            icon={<Group className="w-6 h-6 text-blue-600 dark:text-blue-300" />} 
          />
        </div>
      )}
                      ...
                      >
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-sm">
                          <select
                            ...
                            onChange={(e) => handleUpdateTier(user.id, e.target.value as 'free' | 'premium')}
      ...
      {/* --- NEW --- Admin Announcement Section --- */}
      {!isLoading && !error && (
        <div className="mt-12">
          <h2 className="text-2xl font-bold text-gray-900 dark:text-white mb-4">Admin Announcement</h2>
          <form 
          ...
          >
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
              <div className="md:col-span-1">
                <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2" htmlFor="room-select">
                  Room
                </label>
                <select
                  id="room-select"
