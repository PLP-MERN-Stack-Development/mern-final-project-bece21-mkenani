...
import { 
  Users, Plus, Loader2, AlertTriangle, ChevronRight, LogIn, 
  DoorOpen, BookCopy, ShieldCheck 
} from 'lucide-react'; 
...
// --- MODIFIED --- Interface now includes admin flag
...
// --- NEW --- Interface for a group's rooms
interface GroupRoom {
  id: string;
  group_id: string;
  room_name: string;
...
// --- MODIFIED --- Component now takes a new prop for the new chat UI
interface StudyGroupsProps {
  onSelectGroup: (groupId: string) => void; // For old, simple chat
  onSelectAdminRoom: (groupId: string, roomName: string) => void; // For new, advanced chat
}

const StudyGroups: React.FC<StudyGroupsProps> = ({ onSelectGroup, onSelectAdminRoom }) => {
  useContext(ThemeContext);
  
  // --- MODIFIED --- Split group state
  ...
  const fetchGroups = useCallback(async () => {
    ...
    try {
      ...
      // Split groups into Admin and User groups
      ...
      setEducationLevel(eduLevelRes.data.level); // Save education level
      setError(null);
    } catch (err: any) {
  ...
  // --- NEW --- Handles clicking a room, checks for education level
  ...
  // --- NEW --- Saves the education level from the modal
  const handleEducationLevelSave = async () => {
    ...
    try {
      ...
      await axios.post(`${apiUrl}/user/education-level`, { level }, {
        headers: { Authorization: `Bearer ${token}` }
      });
      
      setEducationLevel(level); // Save locally
  ...
  // Logic for joining a user-created group
  ...
  // Logic for creating a new user group
        ...
        {adminGroup && (
          <div className="bg-white dark:bg-gray-800 rounded-lg shadow-md border border-blue-200 dark:border-blue-700">
            ...
            >
              ...
              <ChevronRight 
                className={`w-5 h-5 text-gray-500 transition-transform ${isAccordionOpen ? 'rotate-90' : 'rotate-0'}`} 
              />
            </button>
              ...
              {isAccordionOpen && (
                ...
                >
                  <div className="border-t border-gray-200 dark:border-gray-700 p-4 space-y-2">
                    ...
                    {!isLoadingRooms && adminRooms.map(room => (
                      <button
                        key={room.id}
        ...
        >
          ...
          >
            ...
            <p className="text-gray-600 dark:text-gray-300 mb-6">
              To join the <strong className="capitalize">{selectedRoom.room_name}</strong> room, 
              please confirm this is your education level. This choice can be updated later in your profile.
            </p>
